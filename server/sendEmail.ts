...
export async function sendResetPasswordEmail(
	...
	return { success: false, error: `EMAIL_PROVIDER inválido: ${emailProvider}` };
}
...
async function sendViaResend(
	...
	if (!resendApiKey.startsWith('re_')) return { success: false, error: 'RESEND_API_KEY inválida' };
	...
	try {
		...
		if (!response.ok) {
			return { success: false, error: `Resend API (${response.status}): ${responseData?.message || 'erro'}` };
		}
		return { success: true };
	} catch (error: any) {
...
async function sendViaSMTP(
	...
	if (smtpService === 'mailgun') return sendViaMailgun(email, resetLink, adminName);
	if (smtpService === 'sendgrid') return sendViaSendGrid(email, resetLink, adminName);
...
async function sendViaSendGrid(
	...
	if (!sendgridApiKey) return { success: false, error: 'SENDGRID_API_KEY é obrigatório' };
	try {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
	<div style="background-color: #f8f9fa; padding: 30px; border-radius: 10px; border: 1px solid #e0e0e0;">
		...
		<div style="text-align: center; margin: 30px 0;">
			<a href="${resetLink}" style="background-color: #3b200d; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; display: inline-block; font-weight: bold;">Redefinir Senha</a>
		</div>
